...
`tests/smoke.js`（115 項冒煙檢查）——每次 PR 由 `.github/workflows/smoke.yml` 自動執行。
...
兩張分析報表、全量備份涵蓋所有集合、全站 KPI 卡點擊總結（總覽/金流/利潤/客戶/佣金）、得標率口徑、寄送方式勾選列印、數量小數、累積估驗明細、工程實績表、材料估算容錯、重複合約點入刪除、既有請款單累計回填、匯出 PDF 先預覽再下載、PDF 大原則（不縮放／四邊10mm／填滿才換頁／續頁表頭／頁碼；工具表單同引擎）、報價單 PDF 排版（分頁連續不重疊、切在列邊界、天地留白、續頁重印表頭、空欄不輸出）。
